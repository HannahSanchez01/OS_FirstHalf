...
void * ThreadChime (void * pData)
{
    ...
    pThreadInfo = (struct ChimeThreadInfo *) pData;
	 ...
	 // Loop the chimes' dings
    while(g_bKeepLooping)
    {
        sleep((unsigned int)(pThreadInfo->fChimeInterval)); // sleep interval defined in struct info
        printf("Ding - Chime %d with an interval of %g s!\n", pThreadInfo->nIndex, pThreadInfo->fChimeInterval);
    }
		...
		as needed */
			...
			if((index == 0) && (strcmp(words[1], "0") != 0)){ // check for invalid input
				...
				if (TheThreads[index].bIsValid == 0){ // if thread is invalid, start a new one!
					...
					if (pthread_create(&(TheThreads[index].ThreadID), NULL, ThreadChime, &TheThreads[index]) != 0){ // Thread creation unsuccessful
						printf("ERROR: thread creation was unsuccessful.\n");
					}
					else{ // Thread creation successful
